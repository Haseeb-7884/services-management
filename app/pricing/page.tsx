  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-14 sm:py-16">
      <div className="mb-12 text-center">
        ...
        <p className="mx-auto mb-8 max-w-md text-base text-[var(--brand-text-muted)] sm:text-lg">Start free, scale when you&apos;re ready. No hidden fees, no lock-in.</p>
        ...
        <div className="flex items-center justify-center gap-3.5">
          ...
          <button
            ...
            style={{ backgroundColor: yearly ? "var(--brand-primary)" : "var(--border-subtle)", borderColor: yearly ? "transparent" : "var(--border-subtle)", boxShadow: yearly ? "0 0 12px var(--glow-primary)" : "none" }}
          >
            <span className="absolute top-[3px] h-[22px] w-[22px] rounded-full bg-white shadow transition-all" style={{ left: yearly ? 26 : 3 }} />
          </button>
          <div className="flex items-center gap-2">
            <span className={`text-sm font-medium ${yearly ? "text-[var(--brand-text)]" : "text-[var(--brand-text-muted)]"}`}>Yearly</span>
            <span className="rounded-full border px-2.5 py-0.5 text-[11px] font-bold" style={{ borderColor: "var(--border-highlight)", backgroundColor: "color-mix(in srgb, var(--brand-primary) 10%, transparent)", color: "var(--brand-primary)" }}>
              Save 28%
            </span>
          </div>
        </div>
      ...
      {loading ? (
        <div className="mb-14 grid grid-cols-1 gap-5 sm:grid-cols-3">
          {Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-[420px] animate-pulse rounded-3xl" style={{ backgroundColor: "var(--surface-card)" }} />)}
        </div>
      ) : error ? (
        ...
        <div className="mb-14 grid grid-cols-1 gap-5 sm:grid-cols-3">
          {plans.map((plan) => (
            ...
            >
              {plan.badge && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full px-4 py-1 text-[11px] font-bold uppercase tracking-wide" style={{ backgroundColor: "var(--brand-primary)", color: "var(--brand-bg-start)", boxShadow: "0 0 16px var(--glow-primary)" }}>
                  {plan.badge}
                </div>
              )}

              <span className="mb-4 inline-block rounded-full px-3.5 py-1 text-[13px] font-bold" style={{ backgroundColor: plan.highlight ? "color-mix(in srgb, var(--brand-primary) 14%, transparent)" : "var(--border-subtle)", color: plan.highlight ? "var(--brand-primary)" : "var(--brand-text-muted)" }}>
                {plan.name}
              </span>
              <div className="mb-1.5 flex items-end gap-1">
                <span className="text-[40px] font-extrabold leading-none tracking-tight text-[var(--brand-text)]">{plan.monthly === 0 ? "Free" : `$${yearly ? plan.yearly : plan.monthly}`}</span>
                {plan.monthly > 0 && <span className="mb-1.5 text-sm text-[var(--brand-text-muted)]">/mo</span>}
              </div>
              {plan.monthly > 0 && yearly && <p className="mb-2 text-xs text-[var(--brand-text-muted)]">Billed yearly · Save ${(plan.monthly - plan.yearly) * 12}/yr</p>}
              ...
              <div className="mb-7 flex flex-col gap-3">
                {plan.features.map((f) => (
                  <div key={f.text} className="flex items-start gap-2.5">
                    <div className="mt-0.5 grid h-[18px] w-[18px] shrink-0 place-items-center rounded-full" style={{ backgroundColor: f.included ? "color-mix(in srgb, var(--brand-primary) 14%, transparent)" : "var(--border-subtle)" }}>
              ...
              <Link
                ...
                style={plan.highlight ? { backgroundColor: "var(--brand-primary)", color: "var(--brand-bg-start)", boxShadow: "0 8px 20px var(--glow-primary)" } : { border: "1.5px solid var(--brand-primary)", color: "var(--brand-primary)", backgroundColor: "transparent" }}
              >
                {plan.cta}
              </Link>
            </div>
      ...
      {!loading && plans.some((p) => p.monthly > 0) && <p className="mb-14 text-center text-xs text-[var(--brand-text-muted)]">Paid-plan checkout isn&apos;t live yet - signing up gets you started on the Free plan today.</p>}

      <div className="mb-14 flex flex-wrap justify-center gap-6 rounded-2xl border p-6" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-card)" }}>
        {TRUST.map(({ icon: Icon, text }) => (
          <div key={text} className="flex items-center gap-2.5 text-[var(--brand-text-muted)]">
            <div className="grid h-9 w-9 place-items-center rounded-lg border" style={{ borderColor: "var(--border-highlight)", backgroundColor: "color-mix(in srgb, var(--brand-primary) 8%, transparent)" }}>
