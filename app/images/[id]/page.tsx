    ...
    <div className="mx-auto max-w-7xl px-4 py-8">
      <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-[1fr_360px]">
        ...
        <div>
          ...
          {image.caption && <h1 className="mb-4 text-xl font-extrabold leading-tight tracking-tight text-[var(--brand-text)]">{image.caption}</h1>}

          <div className="mb-5 flex flex-wrap items-center gap-3.5">
            <Link href={`/u/${image.owner.username}`} className="h-11 w-11 shrink-0 overflow-hidden rounded-full border-2" style={{ borderColor: "var(--border-medium)" }}>
            ...
            <div>
              <Link href={`/u/${image.owner.username}`} className="flex items-center gap-1.5">
        ...
        <div className="lg:sticky lg:top-24">
          ...
          {more.length === 0 ? (
            ...
            <div className="grid grid-cols-3 gap-2">
              {more.map((item) => (
                <Link key={item._id} href={`/images/${item._id}`} className="aspect-square overflow-hidden rounded-lg bg-[var(--brand-bg-start)]">
