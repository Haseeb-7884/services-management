...
export default function Upload() {
  ...
  const submit = async () => {
    ...
    try {
      if (tab === "video") {
        const video = await uploadVideo({ file: file!, title, description, category, tags, onProgress: setProgress });
        router.push(`/videos/${video._id}`);
      } else if (tab === "image") {
        const image = await uploadImage({ file: file!, caption: title, category, tags, onProgress: setProgress });
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10">
      <h1 className="mb-1 text-2xl font-extrabold tracking-tight text-[var(--brand-text)]">{showDetailsStep && tab !== "article" ? "Add details" : "Upload content"}</h1>
      ...
      {!showDetailsStep ? (
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_340px]">
          <div className="space-y-4 rounded-2xl border p-6" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-card)", boxShadow: "var(--shadow-sm)" }}>
            ...
            {submitting && tab !== "article" && (
              <div className="h-1.5 w-full overflow-hidden rounded-full" style={{ backgroundColor: "var(--border-subtle)" }}>
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              >
                {submitting ? (tab !== "article" && progress > 0 && progress < 100 ? `Uploading… ${progress}%` : "Publishing…") : "Publish"}
              </button>
            </div>
          </div>
            ...
            {tab === "article" ? (
              ...
              <div className="overflow-hidden rounded-2xl border" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-card)", boxShadow: "var(--shadow-sm)" }}>
                <div className="aspect-video w-full bg-black">
                  {tab === "video" ? <video src={previewUrl} controls className="h-full w-full object-contain" /> : <img src={previewUrl} alt="Preview" className="h-full w-full object-contain" />}
                </div>
                <div className="p-4">
                  ...
                  {tab === "video" && <p className="text-[11px] leading-relaxed text-[var(--brand-text-muted)]">A thumbnail is generated automatically from your video once it&apos;s published.</p>}
                </div>
              </div>
            )}
          </div>
