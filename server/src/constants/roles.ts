 ...
 * "what can this role do" and is consumed by both the RBAC middleware and
 * (via the /api/v1/config/permissions endpoint, future work) the frontend,
 * so role capabilities can eventually be tuned by the Owner without a
 * redeploy instead of being scattered across `if (role === 'admin')` checks.
 */
...
/** Per-role upload ceilings, enforced in content controllers. Owner can move these to SiteConfig later. */
