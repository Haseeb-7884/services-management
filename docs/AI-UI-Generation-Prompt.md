# AI UI Generation Prompt — Creator Hub Platform (Figma-Make / free-credit friendly)

Copy the box below into Figma Make, Bolt.new, Lovable, or v0.

**On length vs. credit cost:** these aren't the same thing. Figma Make's credit cost is driven by generation *complexity* — number of screens, whether it wires up interactivity/prototyping, whether it builds backend/data logic, how many separate follow-up prompts you send — not by how many words are in the prompt itself. So this version stays detailed enough that nothing about the UI is left to guesswork, while keeping the scope fixed at the same 5 static screens as before (no interactivity, no backend). That combination — thorough spec, narrow scope — is what keeps it inside the free 500/month (150/day cap) budget while still giving Figma exact instructions for every element.

Replace `[PLATFORM NAME]` with whichever name you land on before pasting.
...
- Paste the whole prompt once and let it generate everything in a single pass, rather than building screen-by-screen across separate prompts - each new prompt re-processes prior context, which costs more credits overall than one combined request.
- Skip "Plan Mode" for this pass - it's useful for complex multi-feature apps but adds credit cost on top of the build itself, which isn't worth it for a 5-screen static mockup.
- Don't attach reference files/images unless necessary - attached context gets processed too and adds to the cost.
- After it generates, check your remaining balance (Figma menu → **AI balance**) before asking for more screens or edits, so you know what you've got left.
- If you want more screens later (Explore page, Admin panel, Upload flow, etc.), ask for them **one or two at a time** in small follow-up prompts rather than all at once - easier to control cost, and you can stop once credits run low.

---

## Naming suggestions

A few directions, mixing "creator economy" branding, community-first names, and short coined brand words. None of these have been checked for domain/trademark availability — worth a quick search before committing to one.

- **Orbiq** — creators build their own orbit of followers; short, brandable, techy.
- **Cliqora** — "clique" + a premium "-ora" suffix; leans into tight-knit fan communities.
- **Trybe** — a playful respelling of "tribe"; warm, community-first, easy to say out loud.
- **Beaconhub** — creators as a beacon that draws and holds an audience.
- **Flarecast** — content that "flares up" and spreads/casts outward; energetic, video-friendly.
- **Vantaro** — from "vantage"; aspirational, premium-sounding, works well for a paid-tier product.
- **Nookly** — a cozy "nook" for niche creator communities; approachable, less corporate.
- **Amplyfe** — "amplify" + "life"; leans into creators building a career/life on the platform.

If you want, I can check domain availability and any obvious trademark collisions for your top 2–3 picks before you commit.
