...
export function Navbar() {
  ...
  const runSearch = (query: string) => {
    const q = query.trim();
    setDrawerOpen(false);
    router.push(q ? `/videos?search=${encodeURIComponent(q)}` : "/videos");
  };
  ...
  function isLinkActive(to: string, end: boolean) {
    if (end) return pathname === to;
    return pathname === to || pathname.startsWith(`${to}/`);
  }

  function drawerLinkStyle(to: string, end: boolean) {
    const isActive = isLinkActive(to, end);
    return {
    ...
    <>
        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center gap-2 px-4 sm:gap-3">
          ...
          >
            <div className="flex w-full overflow-hidden rounded-full border" style={{ borderColor: "var(--border-subtle)" }}>
              ...
              />
              <button type="submit" aria-label="Search" className="grid w-12 shrink-0 place-items-center border-l transition hover:bg-[var(--surface-card-hover)]" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--brand-bg-end)" }}>
          ...
          <div className="ml-auto flex items-center gap-1 sm:gap-1.5">
            ...
            {user && (
              <>
                <div className="relative" ref={notifRef}>
                  ...
                  {notifOpen && (
                    <div className="absolute right-0 top-full mt-2 w-80 overflow-hidden rounded-xl border py-1.5" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-card)", boxShadow: "var(--shadow-lg)" }}>
                      ...
                      <div className="max-h-80 overflow-y-auto">
                        {notifications.length === 0 && <p className="px-3.5 py-6 text-center text-sm text-[var(--brand-text-muted)]">No notifications yet.</p>}
                        {notifications.map((n) => {
                      ...
                      <Link href="/notifications" onClick={() => setNotifOpen(false)} className="block border-t px-3.5 py-2.5 text-center text-xs font-semibold text-[var(--brand-primary)]" style={{ borderColor: "var(--border-subtle)" }}>
                        View all
                      </Link>
                    </div>
                  )}
                ...
                <div className="relative ml-0.5" ref={menuRef}>
                  ...
                  {menuOpen && (
                    <div className="absolute right-0 top-full mt-2 w-52 overflow-hidden rounded-xl border py-1.5" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-card)", boxShadow: "var(--shadow-lg)" }}>
                      ...
                      <Link href={`/u/${user.username}`} onClick={() => setMenuOpen(false)} className="flex items-center gap-2.5 px-3.5 py-2 text-sm text-[var(--brand-text)] transition hover:bg-[var(--surface-card-hover)]">
                        <UserIcon size={15} className="text-[var(--brand-text-muted)]" /> View channel
                      </Link>
                      {canSeeDashboard && (
                        <Link href="/dashboard" onClick={() => setMenuOpen(false)} className="flex items-center gap-2.5 px-3.5 py-2 text-sm text-[var(--brand-text)] transition hover:bg-[var(--surface-card-hover)]">
                          <LayoutDashboard size={15} className="text-[var(--brand-text-muted)]" /> Dashboard
                        </Link>
                      )}
                      {canSeeSuperAdmin ? (
                        <Link href="/superadmin" onClick={() => setMenuOpen(false)} className="flex items-center gap-2.5 px-3.5 py-2 text-sm text-[var(--brand-text)] transition hover:bg-[var(--surface-card-hover)]">
                          <ShieldCheck size={15} className="text-[var(--brand-text-muted)]" /> Super Admin
                        </Link>
                      ) : canSeeAdmin ? (
                        <Link href="/admin" onClick={() => setMenuOpen(false)} className="flex items-center gap-2.5 px-3.5 py-2 text-sm text-[var(--brand-text)] transition hover:bg-[var(--surface-card-hover)]">
                          <ShieldCheck size={15} className="text-[var(--brand-text-muted)]" /> Admin
                        </Link>
                      ) : null}
                      <button onClick={handleLogout} className="flex w-full items-center gap-2.5 px-3.5 py-2 text-left text-sm text-[var(--brand-text)] transition hover:bg-[var(--surface-card-hover)]">
            ...
            {!user && (
              <>
                <Link href="/login" className="hidden text-sm font-medium text-[var(--brand-text-muted)] hover:text-[var(--brand-text)] sm:inline">
                  Log in
                </Link>
                <Link href="/register" className="btn-glow rounded-full px-4 py-2 text-sm font-semibold" style={{ backgroundColor: "var(--brand-primary)", color: "var(--brand-bg-start)" }}>
                  Get started
                </Link>
              </>
            )}
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-50">
          ...
          <aside className="absolute left-0 top-0 flex h-full w-72 flex-col overflow-y-auto border-r px-3 py-4" style={{ backgroundColor: "var(--brand-bg-end)", borderColor: "var(--border-subtle)" }}>
            ...
            <nav className="flex flex-col gap-1">
              {NAV_LINKS.map(({ to, label, icon: Icon, end }) => (
                <Link key={to} href={to} onClick={() => setDrawerOpen(false)} className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition" style={drawerLinkStyle(to, end)}>
                  <Icon size={17} />
                  {label}
                </Link>
              ))}
            ...
            {user && (
              <>
                ...
                <nav className="flex flex-col gap-1">
                  <Link href="/upload" onClick={() => setDrawerOpen(false)} className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition" style={drawerLinkStyle("/upload", false)}>
                    <Plus size={17} /> Upload
                  </Link>
                  <Link href="/notifications" onClick={() => setDrawerOpen(false)} className="flex items-center justify-between rounded-lg px-3 py-2.5 text-sm font-medium transition" style={drawerLinkStyle("/notifications", false)}>
                  ...
                  <Link href="/messages" onClick={() => setDrawerOpen(false)} className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition" style={drawerLinkStyle("/messages", false)}>
                    <MessageCircle size={17} /> Messages
                  </Link>
                  {canSeeDashboard && (
                    <Link href="/dashboard" onClick={() => setDrawerOpen(false)} className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition" style={drawerLinkStyle("/dashboard", false)}>
                      <LayoutDashboard size={17} /> Dashboard
                    </Link>
                  )}
                  {canSeeSuperAdmin ? (
                    <Link href="/superadmin" onClick={() => setDrawerOpen(false)} className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition" style={drawerLinkStyle("/superadmin", false)}>
                      <ShieldCheck size={17} /> Super Admin
                    </Link>
                  ) : canSeeAdmin ? (
                    <Link href="/admin" onClick={() => setDrawerOpen(false)} className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition" style={drawerLinkStyle("/admin", false)}>
            ...
            <div className="mt-auto flex flex-col gap-2 pt-4">
              {user ? (
                <>
                  <Link href={`/u/${user.username}`} onClick={() => setDrawerOpen(false)} className="rounded-lg border py-2.5 text-center text-sm font-medium text-[var(--brand-text)]" style={{ borderColor: "var(--border-subtle)" }}>
                    {displayName}&apos;s channel
                  </Link>
                  <button onClick={handleLogout} className="rounded-lg border py-2.5 text-sm font-medium text-[var(--brand-text-muted)]" style={{ borderColor: "var(--border-subtle)" }}>
                    Log out
                  </button>
                </>
              ) : (
                <>
                  <Link href="/login" onClick={() => setDrawerOpen(false)} className="rounded-lg border py-2.5 text-center text-sm font-medium text-[var(--brand-text)]" style={{ borderColor: "var(--border-subtle)" }}>
                    Log in
                  </Link>
                  <Link href="/register" onClick={() => setDrawerOpen(false)} className="rounded-lg py-2.5 text-center text-sm font-semibold" style={{ backgroundColor: "var(--brand-primary)", color: "var(--brand-bg-start)" }}>
                    Get started
                  </Link>
                </>
              )}
