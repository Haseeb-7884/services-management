...
/**
 * Custom branded video player. `object-contain` (not `object-cover`) so the
 * full frame is always visible - portrait/non-16:9 uploads get letterboxed
 * instead of cropped. Every control uses the site's brand color tokens
...
export function VideoPlayer({ src, poster, title }: { src: string; poster?: string; title?: string }) {
  ...
  return (
    ...
    >
      <video ref={videoRef} src={src} poster={poster} className="h-full w-full object-contain" onClick={togglePlay} onDoubleClick={toggleFullscreen} />
      ...
      {!playing && !loading && (
        <button onClick={togglePlay} className="absolute inset-0 grid place-items-center transition" style={{ backgroundColor: "rgba(0,0,0,0.25)" }} aria-label="Play">
          <span className="grid h-16 w-16 place-items-center rounded-full transition hover:scale-105" style={{ backgroundColor: "var(--brand-primary)", boxShadow: "0 8px 24px var(--glow-primary)" }}>
      ...
      >
        ...
        >
          ...
          <div className="absolute top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full opacity-0 shadow transition group-hover/seek:opacity-100" style={{ left: `${progressPct}%`, backgroundColor: "var(--brand-primary)" }} />
        </div>
        ...
        <div className="flex items-center gap-3 text-white">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <input type="range" min={0} max={1} step={0.05} value={muted ? 0 : volume} onChange={(e) => changeVolume(Number(e.target.value))} className="h-1 w-16 accent-white" aria-label="Volume" />
          </div>
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <div className="relative">
              <button onClick={() => setShowSpeedMenu((v) => !v)} className="rounded px-1.5 py-0.5 text-xs font-semibold transition hover:opacity-80">
                {speed}x
              </button>
              {showSpeedMenu && (
                <div className="absolute bottom-full right-0 mb-2 overflow-hidden rounded-lg bg-black/90 py-1 text-xs shadow-lg">
                  {SPEEDS.map((s) => (
                    <button key={s} onClick={() => changeSpeed(s)} className="block w-full whitespace-nowrap px-3.5 py-1.5 text-left transition hover:bg-white/10" style={{ color: s === speed ? "var(--brand-primary)" : "white" }}>
                      {s}x
                    </button>
                  ))}
                </div>
