  ...
  return (
    <div className="space-y-3">
      {items.map((item) => {
        ...
        return (
          <div key={item.id} className="rounded-2xl border p-4" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-card)", boxShadow: "var(--shadow-sm)" }}>
            <div className="flex items-center gap-3.5">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-xs text-[var(--brand-text-muted)]">
                  <span className="uppercase tracking-wide">{item.contentType}</span> · by {ownerName} · {new Date(item.createdAt).toLocaleDateString()}
                </p>
              </div>
