  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="mb-6 space-y-2">
          {holders.map((u) => (
            <div key={u._id} className="flex items-center gap-3 rounded-xl border px-4 py-2.5" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-card)" }}>
              <span className="grid h-9 w-9 shrink-0 place-items-center overflow-hidden rounded-full text-xs font-bold text-white" style={{ background: "linear-gradient(135deg, var(--brand-primary), var(--brand-secondary))" }}>
      ...
      <div className="mb-3 flex gap-2">
        ...
        <button onClick={runSearch} disabled={searching} className="rounded-lg border px-4 py-2 text-sm font-semibold transition hover:bg-[var(--surface-card-hover)]" style={{ borderColor: "var(--border-subtle)" }}>
          Search
        </button>
      </div>
