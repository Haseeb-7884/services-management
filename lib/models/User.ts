...
// `models.User ?? model(...)` (instead of a bare `model()` call) is required
// in Next.js: route handler modules can be re-evaluated on hot-reload/across
// serverless invocations, and mongoose throws "OverwriteModelError" if you
// call `model("User", ...)` a second time in the same process.
