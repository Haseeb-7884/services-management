...
/**
 * Owner-only dashboard for the dynamic branding requirement: site name, logo,
 * favicon, and theme colors, all persisted to SiteConfig and picked up
 * platform-wide the moment they're saved (no redeploy, no hardcoded values).
 */
export function OwnerBrandingSettings() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <p className="mb-7 text-sm text-[var(--brand-text-muted)]">
        Everything here is dynamic - changes apply across the whole site the moment you save, no redeploy needed.
      </p>
        ...
        <div className="space-y-6">
          <section className={cardClass} style={cardStyle}>
            ...
            <div className="flex items-center gap-3">
              <label
                className="cursor-pointer rounded-lg border px-3.5 py-2 text-sm font-medium transition hover:bg-[var(--surface-card-hover)]"
                style={{ borderColor: "var(--border-subtle)" }}
              >
                Upload logo
                <input
          ...
          <section className={cardClass} style={cardStyle}>
            ...
            <input
              ...
              onChange={(e) => setForm((f) => ({ ...f, footer: { ...f.footer, text: e.target.value } }))}
          ...
          <section className={cardClass} style={cardStyle}>
            ...
            <p className="text-xs text-[var(--brand-text-muted)]">
              Used as the fallback page title, description and share-preview image for any page that doesn't set its own (Home, Pricing, etc.).
            </p>
            <div>
          ...
          <button
            onClick={save}
            disabled={saving}
            className="btn-glow w-full rounded-lg py-2.5 text-sm font-bold disabled:opacity-50 sm:w-auto sm:px-8"
        ...
        <div className="lg:sticky lg:top-24 lg:self-start">
          ...
          <div className="overflow-hidden rounded-2xl border" style={{ borderColor: "var(--border-subtle)", boxShadow: "var(--shadow-lg)" }}>
            ...
            <div className="flex items-center gap-2 border-b px-4 py-3" style={{ backgroundColor: colors.backgroundEnd, borderColor: "color-mix(in srgb, " + colors.textMuted + " 25%, transparent)" }}>
              ...
              <span className="text-xs font-semibold" style={{ color: colors.textPrimary }}>{form.siteName || "Your site"}</span>
            </div>
            {/* Mini hero */}
            <div className="p-5" style={{ background: `linear-gradient(160deg, ${colors.backgroundStart}, ${colors.backgroundEnd})` }}>
              <span
                className="mb-3 inline-block rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide"
                style={{ backgroundColor: `color-mix(in srgb, ${colors.primary} 18%, transparent)`, color: colors.primary }}
              >
                Featured
              </span>
          ...
          <p className="mt-3 text-xs leading-relaxed text-[var(--brand-text-muted)]">
            Updates as you edit - this is what visitors will see once you save.
          </p>
        </div>
      </div>
    </div>
