...
const WHY_JOIN = [
  {
    ...
    description: "Videos, shorts, articles, images and live streams all live in one channel - no juggling five different platforms.",
  },
  {
    icon: BarChart3,
    title: "Grow with real insight",
    description: "Audience analytics and content performance that actually explain what's working, not just vanity numbers.",
  },
  {
    icon: ShieldCheck,
    title: "Own your audience",
    description: "Followers, memberships and messages stay tied to your channel - not locked behind an opaque algorithm.",
  },
];
...
function TypeBadge({ type }: { type: FeedItem["type"] }) {
  ...
  return (
    <span
      className="absolute left-2.5 top-2.5 rounded-full border px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider"
      style={{ borderColor: "var(--border-medium)", backgroundColor: "var(--brand-bg-end)", color: "var(--brand-text-muted)" }}
    >
      Article
    </span>
  );
...
export function Home() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      {/* Hero - features the current #1 trending post, or a generic welcome if the platform has no content yet */}
      {hero ? (
        <Link
          to={feedItemHref(hero)}
          className="relative mb-10 flex h-[420px] overflow-hidden rounded-2xl border"
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 max-w-xl p-8 sm:p-10">
            <span
              className="mb-4 inline-block rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-white"
              style={{ backgroundColor: "color-mix(in srgb, var(--brand-primary) 55%, transparent)" }}
            >
            ...
            <div
              className="btn-glow inline-flex items-center gap-2 rounded-lg px-5 py-2.5 text-sm font-bold"
        ...
        !trendingLoading && (
          <div
            className="relative mb-10 flex h-[280px] flex-col items-center justify-center overflow-hidden rounded-2xl border p-8 text-center"
            style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-card)" }}
          >
            <h1 className="font-display mb-3 text-2xl font-extrabold tracking-tight text-[var(--brand-text)] sm:text-3xl">
              Welcome to {branding.siteName}
            </h1>
            <p className="mb-6 max-w-md text-sm text-[var(--brand-text-muted)]">
              Nothing has been published yet - be the first to upload a video, image or article.
            </p>
            <Link
              to="/upload"
              className="btn-glow rounded-lg px-5 py-2.5 text-sm font-bold"
              style={{ backgroundColor: "var(--brand-primary)", color: "var(--brand-bg-start)" }}
            >
              Publish something
            </Link>
      ...
      {(trendingLoading || trending.length > 0) && (
        <section className="mb-10">
          <div className="mb-5 flex items-center justify-between">
            ...
            <Link to="/videos" className="text-sm font-semibold text-[var(--brand-primary)]">
              See all →
            </Link>
          </div>
          <div className="scrollbar-none flex gap-4 overflow-x-auto pb-2">
            {trendingLoading
              ? Array.from({ length: 5 }).map((_, i) => (
                  <div key={i} className="h-[195px] w-[220px] shrink-0 animate-pulse rounded-2xl" style={{ backgroundColor: "var(--surface-card)" }} />
                ))
              : trending.map((item) => (
                  <Link
                    key={item.id}
                    to={feedItemHref(item)}
                    className="card-hover w-[220px] shrink-0 cursor-pointer overflow-hidden rounded-2xl border"
                    ...
                    <div className="relative h-[130px] bg-[var(--brand-bg-start)]">
                      {item.thumbnailUrl ? (
                        ...
                        <div className="grid h-full w-full place-items-center">
                          <FileText size={22} className="text-[var(--brand-text-muted)]" />
                        </div>
                      )}
                    ...
                    <div className="p-3.5">
                      ...
                      <div className="flex items-center justify-between text-xs text-[var(--brand-text-muted)]">
                        ...
                        <span className="flex items-center gap-1">
                          <Eye size={11} /> {formatCount(item.views)}
                        </span>
                      </div>
                    </div>
                  </Link>
          ...
          <div className="scrollbar-none flex gap-5 overflow-x-auto pb-2">
            {creators.map((c) => (
              <div key={c._id} className="flex w-[140px] shrink-0 flex-col items-center gap-2.5 text-center">
                <Link to={`/u/${c.username}`} className="h-20 w-20 overflow-hidden rounded-full border-2" style={{ borderColor: "var(--border-medium)" }}>
                  {c.profile.avatarUrl ? (
                ...
                <div>
                  <Link to={`/u/${c.username}`} className="mb-0.5 block text-[13px] font-semibold text-[var(--brand-text)]">
                    {c.profile.displayName || c.username}
                  </Link>
      ...
      {stats && (
        <div
          className="mb-10 grid grid-cols-2 gap-4 rounded-2xl border p-6 sm:grid-cols-4"
          style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-card)", boxShadow: "var(--shadow-sm)" }}
        >
          {STAT_LABELS.map((s) => (
        ...
        {feedLoading ? (
          <div className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-5">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="h-[330px] animate-pulse rounded-2xl" style={{ backgroundColor: "var(--surface-card)" }} />
            ))}
          </div>
        ) : feed.length === 0 ? (
          <div className="flex flex-col items-center gap-3 rounded-2xl border py-16 text-center" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-card)" }}>
            ...
            <p className="max-w-xs text-sm text-[var(--brand-text-muted)]">
              {activeCategory === "All" ? "No content has been published yet." : `No ${activeCategory.toLowerCase()} published yet.`}
            </p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-5">
              {feed.map((item) => (
                <Link
                  key={item.id}
                  to={feedItemHref(item)}
                  className="card-hover cursor-pointer overflow-hidden rounded-2xl border"
                  style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-card)", boxShadow: "var(--shadow-sm)" }}
                >
                  <div className="relative h-[190px] bg-[var(--brand-bg-start)]">
                    {item.thumbnailUrl ? (
                      ...
                      <div className="grid h-full w-full place-items-center">
                        <FileText size={26} className="text-[var(--brand-text-muted)]" />
                      </div>
                    )}
                    {(item.type === "video" || item.type === "short") && (
                      <>
                        <PlayOverlay />
                        {Boolean(item.durationSec) && (
                          <span className="absolute bottom-2.5 right-2.5 rounded bg-black/80 px-2 py-0.5 text-xs font-semibold text-white">
                            {formatDuration(item.durationSec)}
                          </span>
                        )}
                      </>
                    ...
                    <div className="flex items-center justify-between border-t pt-3" style={{ borderColor: "var(--border-subtle)" }}>
                      <span className="flex items-center gap-2">
                        ...
                        <span>
                          ...
                          <span className="flex items-center gap-1 text-[11px] text-[var(--brand-text-muted)]">
                            <Clock size={10} /> {formatRelativeTime(item.createdAt)}
                          </span>
                        </span>
                      </span>
                      <div className="flex gap-3 text-xs text-[var(--brand-text-muted)]">
                        <span className="flex items-center gap-1">
                          <Heart size={13} /> {formatCount(item.likesCount)}
                        </span>
                        <span className="flex items-center gap-1">
                          <MessageSquare size={13} /> {formatCount(item.commentsCount)}
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            {feedPage < feedPages && (
              <div className="mt-8 text-center">
                <button
                  ...
                  className="rounded-lg border px-6 py-2.5 text-sm font-semibold transition hover:bg-[var(--surface-card-hover)]"
      ...
      <section className="mt-16">
        <div className="mb-8 text-center">
          ...
          <p className="mx-auto max-w-lg text-sm text-[var(--brand-text-muted)]">
            Everything you need to publish, grow and connect with your audience - in one place.
          </p>
        </div>
      ...
      {/* CTA banner - neutral dark panel, accent used only on the button */}
      <section
        className="relative mt-16 overflow-hidden rounded-3xl p-10 text-center sm:p-14"
      ...
      >
        ...
        <p className="relative mx-auto mb-7 max-w-md text-sm text-white/70 sm:text-base">
          Join {branding.siteName} today and start publishing to a home built for creators, not algorithms.
        </p>
        <div className="relative flex flex-wrap items-center justify-center gap-3">
          <Link
            to="/register"
            className="btn-glow flex items-center gap-2 rounded-lg px-6 py-3 text-sm font-bold"
            style={{ backgroundColor: "var(--brand-primary)", color: "var(--brand-bg-start)" }}
          >
            Get started free <ArrowRight size={16} />
          </Link>
          <Link to="/pricing" className="rounded-lg border border-white/20 bg-white/5 px-6 py-3 text-sm font-semibold text-white backdrop-blur-sm">
            View plans
          </Link>
        </div>
      </section>
