...
// There is exactly one top authority - the Owner account, which already
// carries every permission a "Super Admin" would (see ROLE_PERMISSIONS in
// server/src/constants/roles.ts: Owner = every permission, Super Admin is
// a strict subset). This dashboard is that single authority's control
// panel for the platform's 5 Admin slots - it deliberately has no way to
// hand this same level of access to anyone else.
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <p className="mb-6 text-sm text-[var(--brand-text-muted)]">
        Manage Admin slots, Creator status, and content moderation across the platform.
      </p>
      ...
      <div className="scrollbar-none mb-7 flex gap-1 overflow-x-auto rounded-full border p-1" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-card)" }}>
        {TABS.map((t) => (
          <button
            ...
            style={
              tab === t
                ? { backgroundColor: "var(--brand-primary)", color: "var(--brand-bg-start)" }
      ...
      {tab === "Admins" && (
        <RoleSlotManager
          roleLabel="Admin"
          roleFilter="admin"
          slotInfo={stats ? { used: stats.adminSlotsUsed, total: stats.adminSlotsTotal } : undefined}
