  ...
  return (
    <div>
      ...
      <div className="mx-auto max-w-5xl px-4">
        ...
        <div className="relative -mt-12 mb-6">
          <div className="flex flex-wrap items-center gap-5">
            <div className="group relative shrink-0">
              ...
              {isVerified && (
                <div
                  className="absolute bottom-1 right-1 grid h-6 w-6 place-items-center rounded-full border-2"
                  style={{ backgroundColor: "var(--brand-primary)", borderColor: "var(--brand-bg-end)" }}
                >
            ...
            <div className="min-w-[200px] flex-1">
              ...
              {/* YouTube-style single line: @handle · followers · following */}
              ...
              {profile.profile.tagline && (
                <p className="mb-2 mt-1 text-sm font-medium text-[var(--brand-text)]">{profile.profile.tagline}</p>
              )}
              {profile.profile.category && (
                <span
                  className="mb-2 mt-1 inline-block rounded-full border px-2.5 py-0.5 text-[11px] font-semibold"
                  style={{ borderColor: "var(--border-medium)", color: "var(--brand-text-muted)" }}
                >
                  {profile.profile.category}
                </span>
              )}
                ...
                <>
                  ...
                  {viewer && (
                    <Link
                      ...
                      className="rounded-lg border px-3.5 py-2 text-sm font-medium transition hover:bg-[var(--surface-card-hover)]"
        ...
        {/* Content grid */}
        {activeTab === "about" ? (
          <div className="max-w-2xl pb-16 text-sm leading-relaxed text-[var(--brand-text-muted)]">
            {profile.profile.bio || `${name} hasn't added an About section yet.`}
          </div>
        ) : contentLoading ? (
          ...
          <div className="flex flex-col items-center gap-3 rounded-2xl border py-16 text-center" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-card)" }}>
            ...
            <p className="max-w-xs text-sm text-[var(--brand-text-muted)]">
              {isOwnProfile ? "Upload your first post to see it show up here." : `${name} hasn't published anything in this category yet.`}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-5 pb-16">
            {content.map((post) => {
                ...
                >
                  <div className="relative h-[175px] bg-[var(--brand-bg-start)]">
                    {post.thumbnailUrl ? (
                      ...
                      <div className="grid h-full w-full place-items-center">
                        {post.contentType === "article" ? (
                          ...
                          <ImageIcon size={28} className="text-[var(--brand-text-muted)]" />
                        )}
                      </div>
                    )}
                    {isVideoLike && (
                    ...
                    {post.contentType === "short" && (
                      <span
                        className="absolute left-2.5 top-2.5 rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white"
                    ...
                    {post.contentType === "article" && (
                      <span
                        className="absolute left-2.5 top-2.5 rounded-full border px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide"
                  ...
                  <div className="p-4">
                    <h3 className="mb-2.5 line-clamp-2 text-[15px] font-bold leading-snug text-[var(--brand-text)]">
                      {post.title || "Untitled"}
                    </h3>
              ...
              if (isVideoLike) {
                return (
                  <Link key={post.id} to={`/videos/${post.id}`}>
                    {card}
                  </Link>
                );
              }
              if (post.contentType === "image") {
                return (
                  <Link key={post.id} to={`/images/${post.id}`}>
                    {card}
                  </Link>
                );
              }
