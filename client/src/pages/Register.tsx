  ...
  const onSubmit = async (values: RegisterFormValues) => {
    ...
    try {
      await registerUser({
        ...
        // Only send a display name if the user actually typed one - an empty
        // string here previously tripped the server's validation and showed
        // as a generic "registration failed".
    ...
    <AuthCard
      ...
      footer={
        <>
          ...
          <Link to="/login" className="font-medium text-[var(--brand-primary)] hover:underline">
            Log in
          </Link>
        </>
      }
    >
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
        ...
        <button
          ...
          className="btn-glow flex w-full items-center justify-center gap-2 rounded-lg py-2.5 text-sm font-semibold disabled:opacity-60"
