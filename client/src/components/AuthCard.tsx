...
export function AuthCard({
  ...
  return (
    <div className="relative flex min-h-[calc(100vh-4rem)] items-center overflow-hidden px-4 py-10">
      {/* Decorative background - fills the empty space around the card
          instead of leaving a flat void: soft blurred accent blobs + a
          faint dot grid, a common pattern on premium SaaS auth pages. */}
      <div className="pointer-events-none absolute inset-0">
        <div
          className="absolute -left-16 top-[8%] h-72 w-72 rounded-full opacity-[0.14] blur-3xl"
          style={{ backgroundColor: "var(--brand-primary)" }}
        />
        <div
          className="absolute -right-20 bottom-[6%] h-80 w-80 rounded-full opacity-[0.10] blur-3xl"
          style={{ backgroundColor: "var(--brand-secondary)" }}
        />
        <div
          className="absolute inset-0 opacity-60"
          style={{ backgroundImage: "radial-gradient(var(--border-medium) 1px, transparent 1px)", backgroundSize: "28px 28px" }}
        />
      </div>
      ...
      >
        {/* Branded panel - neutral dark, large screens only. The accent color
            pops against it instead of being the whole background. */}
        <div
        ...
        >
          ...
          <Link to="/" className="relative flex items-center gap-2 text-lg font-bold text-white">
            {branding.logoUrl ? (
          ...
          <div className="relative">
            <h2 className="font-display mb-5 text-2xl font-extrabold leading-tight text-white">
              A home for your content, built for creators.
            </h2>
                  ...
                  {proofAvatars.map((c) =>
                    c.profile.avatarUrl ? (
                      <img
                        ...
                        alt={c.profile.displayName || c.username}
                        className="h-8 w-8 rounded-full border-2 object-cover"
                        style={{ borderColor: "color-mix(in srgb, var(--brand-text) 80%, black)" }}
                      />
                    ) : (
                      <span
                        key={c._id}
                        className="grid h-8 w-8 place-items-center rounded-full border-2 text-xs font-bold text-white"
                        style={{ borderColor: "color-mix(in srgb, var(--brand-text) 80%, black)", backgroundColor: "var(--brand-primary)" }}
                      >
        ...
        <div className="flex flex-col justify-center p-8 sm:p-10" style={{ backgroundColor: "var(--surface-card)" }}>
          <Link to="/" className="mb-8 flex items-center gap-2 text-lg font-semibold lg:hidden">
            {branding.logoUrl ? (
